 ...
 * 输入的参数   <input path><output path> <date> <timepoint>
 *     参数实例  “/base /output 2012-09-12 07-17-24”
 *
 */
public class Task2 extends Configured implements Tool {
    ...
    public static class Map2 extends Mapper<LongWritable,Text,Text,NewObject>{
    ...
    public static class Reduce2 extends Reducer<Text,NewObject,NullWritable,Text>{
        ...
        protected void reduce(Text key, Iterable<NewObject> values, Context context) throws IOException, InterruptedException {
            ...
            for (NewObject no:values){
                    list.add(no.clone());//特别注意，在reduce中，key和values都只有一个对象生成，如果不用clone()的话，则都是最后一个对象的值
